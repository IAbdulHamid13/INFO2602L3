...
class Category(db.Model):
    ...
    user = db.relationship(
        "RegularUser", backref=db.backref("categories", lazy="joined")
    )
    todos = db.relationship(
        "Todo", secondary="todo_category", backref=db.backref("categories", lazy=True)
    )
...
class RegularUser(User):
    ...
    todos = db.relationship(
        "Todo", backref="user", lazy=True
    )  # sets up a relationship to todos which references User
    __mapper_args__ = {
